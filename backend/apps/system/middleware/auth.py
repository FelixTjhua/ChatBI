...
class TokenMiddleware(BaseHTTPMiddleware):
    ...
    async def validateToken(self, token: Optional[str], trans: I18n):
        ...
        try: 
            ...
            msg = str(e)
            ChatBILogUtil.error(f"Token validation error: {msg}")
            # 不向客户端泄露内部异常详情，使用通用错误消息
            if 'expired' in msg.lower():
                return False, str(trans('i18n_permission.token_expired'))
    ...
    async def validateAssistant(self, assistantToken: Optional[str], trans: I18n) -> tuple[any]:
        ...
        try:
            ...
            if schema.lower() != "assistant":
                return False, f"Token schema error!", None
            payload = jwt.decode(
                param, settings.SECRET_KEY, algorithms=[security.ALGORITHM]
            )
            token_data = TokenPayload(**payload)
            if not payload.get('assistant_id'):
                return False, f"Miss assistant payload error!", None
            with Session(engine) as session:
                """ session_user = await get_user_info(session = session, user_id = token_data.id)
                session_user = UserInfoDTO.model_validate(session_user) """
            ...
            ChatBILogUtil.exception(f"Assistant validation error: {str(e)}")
            # 不向客户端泄露内部异常详情
            return False, "Assistant token validation failed", None
    
    async def validateEmbedded(self, param: str, trans: I18n) -> tuple[any]:
        try: 
            # 先验证JWT签名，确保token未被篡改
            ...
            # 验证 embeddedId 为有效正整数，防止类型混淆或无效值
            try:
                embeddedId = int(embeddedId)
                if embeddedId <= 0:
                    return False, "Invalid embedded ID", None
            except (TypeError, ValueError):
                return False, "Invalid embedded ID format", None
            if not payload.get('account'):
                return False, f"Miss account payload error!", None
            ...
            ChatBILogUtil.exception(f"Embedded validation error: {str(e)}")
            # 不向客户端泄露内部异常详情
            return False, "Embedded token validation failed", None
    
def xor_decrypt(encrypted_str: str, key: int = 0xABCD1234) -> int:
    """从 appKey 中提取 embeddedId。"""
    try:
        ...
        # 解码失败时返回 -1 而非抛出未处理异常
        # 上层 validateEmbedded 会检查 embeddedId <= 0 并拒绝
        return -1