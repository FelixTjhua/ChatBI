...
from common.utils.crypto import chatbi_encrypt
...
async def async_model_info():
    with Session(engine) as session:
        ...
        if model_list:
            for model in model_list:
                ...
                if model.api_domain.startswith("http"):
                    if model.api_key:
                        model.api_key = await chatbi_encrypt(model.api_key)
                    if model.api_domain:
                        model.api_domain = await chatbi_encrypt(model.api_domain)
        ...
        if any_model_change:
            session.commit()
            ChatBILogUtil.info("异步加密已有模型的密钥和地址完成")           
