...
from common.core.chatbi_cache import cache
...
def get_assistant_user(*, id: int):
    return UserInfoDTO(id=id, account="chatbi-inner-assistant", name="chatbi-inner-assistant",
                       email="chatbi-inner-assistant@chatbi.com", role="admin")
...
def init_dynamic_cors(app: FastAPI):
    """初始化动态CORS（嵌入式助手功能已移除，保留空实现以兼容启动流程）"""
    pass
...
class AssistantOutDs:
    ...
    def get_db_schema(self, ds_id: int, question: str, embedding: bool = True) -> str:
        ...
        # do table embedding (disabled for assistant mode)
    ...
    def convert2schema(self, ds_dict: dict, config: dict[any]) -> AssistantOutDsSchema:
        ...
        for attr in attr_list:
            if attr in ds_dict:
                id_marker += str(ds_dict.get(attr, '')) + '--chatbi--'
