...
def custom_key_builder(
    ...
    *,
    ...
    try:
        ...
        ChatBILogUtil.error(f"Key builder error: {str(e)}")
        ...
        async def wrapper(*args, **kwargs):
            ...
            for temp_cache_key in key_list:
                if await backend.get(temp_cache_key):
                    ...
                    ChatBILogUtil.debug(f"Cache cleared: {temp_cache_key}")
            return await func(*args, **kwargs)
...
def init_chatbi_cache():
    cache_type: str = settings.CACHE_TYPE
    if cache_type == "memory":
        FastAPICache.init(InMemoryBackend())
        ChatBILogUtil.info("ChatBI 使用内存缓存, 仅支持单进程模式")
        ...
        FastAPICache.init(RedisBackend(redis_client), prefix="chatbi-cache")
        ChatBILogUtil.info(f"ChatBI 使用Redis缓存, 可使用多进程模式")
    else:
        ChatBILogUtil.warning("ChatBI 未启用缓存, 可使用多进程模式")
...
def is_cache_initialized() -> bool:
    ...
    try:
        ...
        ChatBILogUtil.debug(f"缓存初始化检查失败: {str(e)}")
        return False
