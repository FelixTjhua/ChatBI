...
async def get_current_assistant(request: Request) -> AssistantHeader | None:
    base_assistant = getattr(request.state, 'assistant', None)
    if base_assistant is None:
        return None
    if request.headers.get("X-CHATBI-ASSISTANT-CERTIFICATE"):
        entry_certificate = request.headers['X-CHATBI-ASSISTANT-CERTIFICATE']
        base_assistant.certificate = unquote(base64.b64decode(entry_certificate).decode('utf-8'))
    if request.headers.get("X-CHATBI-ASSISTANT-ONLINE"):
        entry_online = request.headers['X-CHATBI-ASSISTANT-ONLINE']
