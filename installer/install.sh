...
function check_and_prepare_env_params() {
    ...
    if [ -f /usr/bin/sctl ]; then
        ...
        CHATBI_BASE_OLD=${CHATBI_BASE}
        sed -i -e "s#CHATBI_BASE=.*#CHATBI_BASE=${CHATBI_BASE}#g" sctl
        \cp sctl /usr/local/bin && chmod +x /usr/local/bin/sctl

        log_content "停止 ChatBI 服务"
        sctl stop
    ...
    if [[ ${CHATBI_BASE_OLD} ]];then
        CHATBI_BASE=${CHATBI_BASE_OLD}
        export CHATBI_BASE=${CHATBI_BASE_OLD}
    fi
    if [[ -d ${CHATBI_BASE} ]] && [[ -f ${CHATBI_BASE}/chatbi/.env ]]; then
        source $CHATBI_BASE/chatbi/.env
        INSTALL_TYPE='upgrade'
        log_content "升级安装"
    else
        INSTALL_TYPE='install'
...
function set_run_base_path() {
    log_title "设置运行目录"
    CHATBI_RUN_BASE=$CHATBI_BASE/chatbi
    CONF_FOLDER=${CHATBI_RUN_BASE}/conf
    TEMPLATES_FOLDER=${CHATBI_RUN_BASE}/templates
    log_content "运行目录 $CHATBI_RUN_BASE"
    log_content "配置文件目录 $CONF_FOLDER"
}

function prepare_chatbi_run_base() {
    log_title "初始化运行目录"
    cd ${CURRENT_DIR}
    mkdir -p ${CHATBI_RUN_BASE}
    log_content "复制安装文件到运行目录"
    cp -r ./chatbi/* ${CHATBI_RUN_BASE}/

    cd ${CHATBI_RUN_BASE}
    env | grep CHATBI_ >.env

    mkdir -p ${CHATBI_RUN_BASE}/conf
    mkdir -p ${CHATBI_RUN_BASE}/data/chatbi/{excel,images,logs}

    if [ "${CHATBI_EXTERNAL_DB}" = "false" ]; then
        mkdir -p ${CHATBI_RUN_BASE}/data/postgresql
        export CHATBI_DB_PORT=5432
    else
...
function prepare_system_settings() {
    ...
    if which firewall-cmd >/dev/null 2>&1; then
        if systemctl is-active firewalld &>/dev/null ;then
            log_content "开启防火墙端口 ${CHATBI_WEB_PORT}"
            firewall-cmd --zone=public --add-port=${CHATBI_WEB_PORT}/tcp --permanent
            log_content "开启防火墙端口 ${CHATBI_MCP_PORT}"
            firewall-cmd --zone=public --add-port=${CHATBI_MCP_PORT}/tcp --permanent
            firewall-cmd --reload
        else
...
function load_images() {
    log_title "加载 ChatBI 镜像"
    cd ${CURRENT_DIR}

    for i in $(docker images --format '{{.Repository}}:{{.Tag}}' | grep chatbi); do
    ...
    if [[ -d images ]]; then
       ...
       CHATBIVERSION=$(cat ${CURRENT_DIR}/chatbi/templates/version)
       log_content "ChatBI 版本: ${CHATBIVERSION}"
    fi
}

function start_chatbi() {
    log_title "启动 ChatBI 服务"
    sctl reload 2>&1 | tee -a ${CURRENT_DIR}/install.log
    if [[ $? -ne 0 ]]; then
        log_content "ChatBI 服务启动失败，请检查日志"
        exit 1
    fi
    echo
    if [[ $INSTALL_TYPE != "upgrade" ]];then
       echo -e "======================= 安装完成 =======================\n" 2>&1 | tee -a ${CURRENT_DIR}/install.log
       echo -e "系统登录信息如下:\n\t访问地址: http://服务器IP:$CHATBI_WEB_PORT\n\t用户名: admin\n\t初始密码: ChatBI@123456" 2>&1 | tee -a ${CURRENT_DIR}/install.log
    else
       echo -e "======================= 升级完成 =======================\n" 2>&1 | tee -a ${CURRENT_DIR}/install.log
    fi
}
