;(function () {
  window.chatbi_assistant_handler = window.chatbi_assistant_handler || {}
  const defaultData = {
    id: '1',
    show_guide: false,
    float_icon: '',
  ...
  const script_id_prefix = 'chatbi-assistant-float-script-'
  const guideHtml = `
<div class="chatbi-assistant-mask">
  <div class="chatbi-assistant-content"></div>
</div>
<div class="chatbi-assistant-tips">
  <div class="chatbi-assistant-close">
  ...
  <div class="chatbi-assistant-title"> 🌟 遇见问题，不再有障碍！</div>
  <p>你好，我是你的智能小助手。<br/>
      点我，开启高效解答模式，让问题变成过去式。</p>
  <div class="chatbi-assistant-button">
      <button>我知道了</button>
  </div>
  <span class="chatbi-assistant-arrow" ></span>
</div>
`

  const chatButtonHtml = (data) => `
<div class="chatbi-assistant-chat-button">
  ...
  const getChatContainerHtml = (data) => {
    ...
    return `
<div id="chatbi-assistant-chat-container">
  <iframe id="chatbi-assistant-chat-iframe-${data.id}" allow="microphone;clipboard-read 'src'; clipboard-write 'src'" src="${srcUrl}"></iframe>
  <div class="chatbi-assistant-operate">
  <div class="chatbi-assistant-closeviewport chatbi-assistant-viewportnone">
  ...
  <div class="chatbi-assistant-openviewport">
    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
  ...
  <div class="chatbi-assistant-chat-close">
    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
...
`
  ...
  const initGuide = (root) => {
    ...
    const button = root.querySelector('.chatbi-assistant-button')
    const close_icon = root.querySelector('.chatbi-assistant-close')
    const close_func = () => {
      root.removeChild(root.querySelector('.chatbi-assistant-tips'))
      root.removeChild(root.querySelector('.chatbi-assistant-mask'))
      localStorage.setItem('chatbi_assistant_mask_tip', true)
    }
  ...
  const initChat = (root, data) => {
    ...
    // 按钮元素
    const chat_button = root.querySelector('.chatbi-assistant-chat-button')
    let chat_button_img = root.querySelector('.chatbi-assistant-chat-button > svg')
    if (data.float_icon) {
      chat_button_img = root.querySelector('.chatbi-assistant-chat-button > img')
    }
    chat_button_img.style.display = 'block'
    //  对话框元素
    const chat_container = root.querySelector('#chatbi-assistant-chat-container')
    // 引导层
    const mask_content = root.querySelector('.chatbi-assistant-mask > .chatbi-assistant-content')
    const mask_tips = root.querySelector('.chatbi-assistant-tips')
    ...
    const viewport = root.querySelector('.chatbi-assistant-openviewport')
    const closeviewport = root.querySelector('.chatbi-assistant-closeviewport')
    const close_func = () => {
      chat_container.style['display'] =
        chat_container.style['display'] == 'block' ? 'none' : 'block'
      chat_button.style['display'] = chat_container.style['display'] == 'block' ? 'none' : 'block'
    }
    close_icon = chat_container.querySelector('.chatbi-assistant-chat-close')
    chat_button.onclick = close_func
    close_icon.onclick = close_func
    const viewport_func = () => {
      if (chat_container.classList.contains('chatbi-assistant-enlarge')) {
        chat_container.classList.remove('chatbi-assistant-enlarge')
        viewport.classList.remove('chatbi-assistant-viewportnone')
        closeviewport.classList.add('chatbi-assistant-viewportnone')
      } else {
        chat_container.classList.add('chatbi-assistant-enlarge')
        viewport.classList.add('chatbi-assistant-viewportnone')
        closeviewport.classList.remove('chatbi-assistant-viewportnone')
      }
    }
   ...
   * 第一次进来的引导提示
   */
  function initchatbi_assistant(data) {
    const chatbi_div = document.createElement('div')
    const root = document.createElement('div')
    const chatbi_root_id = 'chatbi-assistant-root-' + data.id
    root.id = chatbi_root_id
    initchatbi_assistantStyle(chatbi_div, chatbi_root_id, data)
    chatbi_div.appendChild(root)
    document.body.appendChild(chatbi_div)
    const chatbi_assistant_mask_tip = localStorage.getItem('chatbi_assistant_mask_tip')
    if (chatbi_assistant_mask_tip == null && data.show_guide) {
      initGuide(root)
    }
    initChat(root, data)
  }
  ...
  // 初始化全局样式
  function initchatbi_assistantStyle(root, chatbi_assistantId, data) {
    ...
    style.innerText = `
  /* 放大 */
  #chatbi-assistant .chatbi-assistant-enlarge {
      width: 50%!important;
  ...
  #chatbi-assistant .chatbi-assistant-mask {
      position: fixed;
      z-index: ${maskZIndex};
  ...
  #chatbi-assistant .chatbi-assistant-mask .chatbi-assistant-content {
      width: 64px;
      height: 64px;
  ...
  #chatbi-assistant .chatbi-assistant-tips {
      position: fixed;
  ...
  #chatbi-assistant .chatbi-assistant-tips .chatbi-assistant-arrow {
      position: absolute;
      background: #3370FF;
      width: 10px;
      height: 10px;
  ...
  #chatbi-assistant .chatbi-assistant-tips .chatbi-assistant-title {
      font-size: 20px;
      font-weight: 500;
      margin-bottom: 8px;
  }
  #chatbi-assistant .chatbi-assistant-tips .chatbi-assistant-button {
      text-align: right;
      margin-top: 24px;
  }
  #chatbi-assistant .chatbi-assistant-tips .chatbi-assistant-button button {
      border-radius: 4px;
      background: #FFF;
      padding: 3px 12px;
      color: #3370FF;
  ...
  #chatbi-assistant .chatbi-assistant-tips .chatbi-assistant-button button::after{
      border: none;
    }
  #chatbi-assistant .chatbi-assistant-tips .chatbi-assistant-close {
      position: absolute;
      right: 20px;
      top: 20px;
      cursor: pointer;
  ...
  #chatbi-assistant-chat-container {
        width: 460px;
        height: 640px;
        display:none;
      }
  ...
  #chatbi-assistant .chatbi-assistant-chat-button{
    position: fixed;
    ${data.x_type}: ${data.x_val}px;
    ${data.y_type}: ${data.y_val}px;
    cursor: pointer;
  ...
  #chatbi-assistant #chatbi-assistant-chat-container{
    z-index: ${zIndex};
    position: relative;
    border-radius: 8px;
  ...
  #chatbi-assistant #chatbi-assistant-chat-container .chatbi-assistant-operate{
    top: 18px;
    right: 15px;
    position: absolute;
    display: flex;
  ...
  #chatbi-assistant #chatbi-assistant-chat-container .chatbi-assistant-operate .chatbi-assistant-chat-close{
    margin-left:15px;
    cursor: pointer;
  }
  #chatbi-assistant #chatbi-assistant-chat-container .chatbi-assistant-operate .chatbi-assistant-openviewport{

    cursor: pointer;
  }
  #chatbi-assistant #chatbi-assistant-chat-container .chatbi-assistant-operate .chatbi-assistant-closeviewport{

    cursor: pointer;
  }
  #chatbi-assistant #chatbi-assistant-chat-container .chatbi-assistant-viewportnone{
    display:none;
  }
  #chatbi-assistant #chatbi-assistant-chat-container #chatbi-assistant-chat-iframe-${data.id} {
    height:100%;
    width:100%;
    border: none;
  }
  ...
  }`.replaceAll('#chatbi-assistant ', `#${chatbi_assistantId} `)
  ...
  function registerMessageEvent(id, data) {
    const iframe = document.getElementById(`chatbi-assistant-chat-iframe-${id}`)
    const url = iframe.src
    const eventName = 'chatbi_assistant_event'
  ...
  function init() {
    const chatbiScripts = document.querySelectorAll(`script[id^="${script_id_prefix}"]`)
    const scriptsArray = Array.from(chatbiScripts)
    const src_list = scriptsArray.map((script) => script.src)
    src_list.forEach((src) => {
      const id = getParam(src, 'id')
      window.chatbi_assistant_handler[id] = window.chatbi_assistant_handler[id] || {}
      window.chatbi_assistant_handler[id]['id'] = id
  ...
  function expposeGlobalMethods(id) {
    window.chatbi_assistant_handler[id]['setOnline'] = (online) => {
    ...
    window.chatbi_assistant_handler[id]['refresh'] = (online, userFlag) => {
      if (online != null && typeof online != 'boolean') {
        throw new Error('The parameter can only be of type boolean')
      }
      const iframe = document.getElementById(`chatbi-assistant-chat-iframe-${id}`)
    ...
    window.chatbi_assistant_handler[id]['destroy'] = () => {
      const chatbi_root_id = 'chatbi-assistant-root-' + id
      const container_div = document.getElementById(chatbi_root_id)
      if (container_div) {
      ...
      const scriptDom = document.getElementById(`chatbi-assistant-float-script-${id}`)
      if (scriptDom) {
      ...
      delete window.chatbi_assistant_handler[id]
    }
    window.chatbi_assistant_handler[id]['setHistory'] = (show) => {
    ...
    window.chatbi_assistant_handler[id]['createConversation'] = (param) => {
      const iframe = document.getElementById(`chatbi-assistant-chat-iframe-${id}`)
      if (iframe) {
        const url = iframe.src
        const eventName = 'chatbi_assistant_event'
