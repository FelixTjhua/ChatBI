...
const props = withDefaults(
  defineProps<{
    stage: 'idle' | 'query' | 'rag' | 'augment' | 'loading' | 'predicting' | 'generating' | 'chart' | 'done' | 'failed'
    errorMessage?: string
  }>(),
  {
    stage: 'idle',
...
const stages = computed(() => [
  {
    ...
    desc: t('predict_process.ai_predict_desc'),
    detail: t('thinking.rag_detail_trend_prediction'),
    show: true,
    ragPhase: 'G',
  },
])
...
<template>
  <div v-if="stage !== 'idle' && stage !== 'done'" class="predict-process-indicator">
    <div class="process-header">
      <div class="header-glow"></div>
      <span class="header-icon">{{ stage === 'failed' ? '❌' : '📈' }}</span>
      <span class="header-text">{{ stage === 'failed' ? t('predict_process.process_failed') : t('predict_process.processing') }}</span>
